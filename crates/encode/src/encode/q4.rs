    ...
    while pos + HASH_TYPE_LEN < pos_end {
        ...
        let Some(mut found) = find_match::<TABLE_BITS, HASH_LEN, SKIP_DICT_AFTER_MATCH>(
            ...
            if pos > apply_sparse_search {
                pos = skip_sparse::<TABLE_BITS, HASH_LEN>(
                    input,
                    &mut table,
                    pos,
                    pos_end,
        ...
        store_range::<TABLE_BITS, HASH_LEN>(
            input,
            &mut table,
            pos + 2,
...
fn find_match<const TABLE_BITS: usize, const HASH_LEN: usize, const SKIP_DICT_AFTER_MATCH: bool>(
    ...
    let key = hash::<TABLE_BITS, HASH_LEN>(input, pos);
...
fn store_range<const TABLE_BITS: usize, const HASH_LEN: usize>(
    input: &[u8],
    table: &mut [u32],
    start: usize,
    end: usize,
...
fn store<const TABLE_BITS: usize, const HASH_LEN: usize>(
    input: &[u8],
    table: &mut [u32],
    pos: usize,
) {
...
#[inline(always)]
fn hash<const TABLE_BITS: usize, const HASH_LEN: usize>(input: &[u8], pos: usize) -> usize {
